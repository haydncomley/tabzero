...
export const tool = onCall(
	...
	async (request) => {
		...
		const { action, recentMessages } = request.data as {
		...
		for (const result of resultsMapped) {
			...
			if (toolIndex !== -1) {
				if (typeof result.result?.success !== 'undefined') {
					log.tools[toolIndex].status = result.result.success
						? 'success'
						: 'error';
				} else {
				...
				if (result.result?.link) {
					log.tools[toolIndex].link = result.result?.link;
				}
			}
		}
