...
import { firestore, openaiKey } from '../config';
import { FieldValue } from 'firebase-admin/firestore';

export const aiChat = onCall({ secrets: [openaiKey] }, async (request) => {
...
export const aiTranscribe = onCall({ secrets: [openaiKey] }, async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'User must be authenticated');
...
export const aiSpeak = onCall({ secrets: [openaiKey] }, async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'User must be authenticated');
...
export const aiToolResolver = onCall({ secrets: [openaiKey] }, async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'User must be authenticated');
