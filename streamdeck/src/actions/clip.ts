...
const STATE_IMAGE_MAP = {
	connected: 'imgs/actions/clip/connected.png',
	disconnected: 'imgs/actions/clip/disconnected.png',
} as const;

const connectToTabZero = (
	ev: WillAppearEvent<ClipSettings> | KeyDownEvent<ClipSettings>,
) => {
 ...
 */
@action({ UUID: 'com.haydn-comley.tabzero.clip' })
export class Clip extends SingletonAction<ClipSettings> {
	/**
	...
	override onWillAppear(
		ev: WillAppearEvent<ClipSettings>,
	): void | Promise<void> {
		ev.action.setImage(STATE_IMAGE_MAP.disconnected);
	...
	override onWillDisappear(
		ev: WillDisappearEvent<ClipSettings>,
	): Promise<void> | void {
		wsClient?.close();
	}
